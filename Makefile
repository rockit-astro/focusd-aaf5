...
all:
	...
	${RPMBUILD} --define "_version %(cat VERSION)" -ba rockit-focuser-aaf5.spec
	${RPMBUILD} --define "_version %(cat VERSION)" -ba python3-rockit-focuser-aaf5.spec
...
install:
	...
	@sudo pip3 install rockit.focuser.aaf5-$$(cat VERSION)-py3-none-any.whl
	@rm VERSION
	@cp focusd focus /bin/
